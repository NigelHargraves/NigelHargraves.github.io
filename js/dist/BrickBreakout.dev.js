...
window.addEventListener("load", function () {
  ...
  var speed = canvas.width / 300;
  var paddleWidth;
  if (canvas.width < 500) paddleWidth = 50;else paddleWidth = 100;
  var paddle, ball;
  var moveLeft = false,
      moveRight = false,
      gameOver = false,
    ...
    if (ball.x - ball.r > paddle.x && ball.x + ball.r < paddle.x + paddle.w && ball.y + ball.r > paddle.y && !ballHitPaddle) {
      ...
      if (!ballHitPaddle) {
        if (moveLeft || moveRight) {
          ...
          if (ball.velocity.y > speed / 3) {
            ball.velocity.y -= speed / 6;
          }
        } else {
          wallBounce.play();
          ballHitPaddle = true;
  ...
  function init() {
    paddle = new Paddle(canvas.width / 2 - 50, canvas.height - 40, paddleWidth, canvas.height / 100, "white");
    paddle.update();
